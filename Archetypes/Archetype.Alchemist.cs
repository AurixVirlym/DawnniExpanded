...
public static class ArchetypeAlchemist
{
  ...
  public static Trait InfusedTrait = ModManager.RegisterTrait(
        "DE_Infused",
        new TraitProperties("Infused", true, "An infused item is expires at the end of an encounter and is removed ", true)
        {
        });

  public static readonly Trait AlchemistDedicationTrait = ModManager.RegisterTrait(
    "DE_AlchemistTrait", new TraitProperties("Alchemist", false));
  ...
  {
    AlchemistDedicationFeat = ArchetypeFeats.CreateAgnosticArchetypeDedication(AlchemistDedicationTrait, "You enjoy tinkering with alchemical formulas and substances in your spare time, and your studies have progressed beyond mere experimentation.",
        "\b\bYou gain a infused reagents equal to your level and the Quick Alchemy {icon:Action} action which can be used to make any level 1 bomb, elixir, or mutagen using 1 infused reagents. These items only last until the end of an encounter.\n\nInfused Reagents are restored after a long rest.")
            ...
            }).WithOnCreature((CalculatedCharacterSheetValues sheet, Creature creature) =>
        {
          creature.AddQEffect(new QEffect("Quick Alchemy", "You can quickly create alchemical items.")
          {
            ProvideActionIntoPossibilitySection = (QEffect qfself, PossibilitySection possibilitySection1) =>
            {
              ...
              PossibilitySection BombSection = new PossibilitySection("Bombs");
              ...
              foreach (Item AlchemyItem in Items.ShopItems.Where<Item>((Func<Item, bool>)(item =>
                         item.Level <= AlchemyLevel && item.HasTrait(Trait.Bomb)))
                      )
              {
                BombSection.AddPossibility(CreateAlchemyPossibilityItem(AlchemyItem, creature));
              }
            ...
            "Quick Alchemy");
              ...
              submenuPossibility.Subsections.Add(BombSection);
    ...
    AlchemistDedicationFeat.Traits.Add(DawnniExpanded.DETrait);

    ExpertAlchemyFeat = new TrueFeat(ModManager.RegisterFeatName("DE_ExpertAlchemy", "Expert Alchemy"),
    4,
    "You have learned how to brew stronger elixirs.",
    ...
    new Trait[] { FeatArchetype.ArchetypeTrait, DawnniExpanded.DETrait })
      .WithAvailableAsArchetypeFeat(AlchemistDedicationTrait)
